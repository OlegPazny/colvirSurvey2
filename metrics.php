...
session_start();
if ($_SESSION['userType'] !== "admin" && $_SESSION['userType'] !== "supervisor") {
...
SELECT * FROM (
    ...
    SELECT 
        'Технический департамент' AS department_name,
        1 AS order_column,
    ...
    FROM departments d
    WHERE d.id IN (2) -- Укажите реальные ID для тех. департамента

    UNION ALL

    -- 2. Производственный департамент
    SELECT 
        'Производственный департамент' AS department_name,
        2 AS order_column,
        SUM(d.current) AS total_employees_current,
        SUM(d.prev) AS total_employees_prev,
        SUM((SELECT COUNT(*) FROM results r WHERE r.q1 = d.id)) AS current_participants,
        SUM((SELECT COUNT(*) FROM prev_results pr WHERE pr.q1 = d.id)) AS previous_participants
    FROM departments d
    WHERE d.id IN (3, 4, 5, 7)

    UNION ALL

    -- 3. Департамент развития бизнеса и продуктов
    SELECT 
        'Департамент развития бизнеса и продуктов' AS department_name,
        3 AS order_column,
        SUM(d.current) AS total_employees_current,
        SUM(d.prev) AS total_employees_prev,
        SUM((SELECT COUNT(*) FROM results r WHERE r.q1 = d.id)) AS current_participants,
        SUM((SELECT COUNT(*) FROM prev_results pr WHERE pr.q1 = d.id)) AS previous_participants
    FROM departments d
    WHERE d.id IN (8, 9, 10)

    UNION ALL

    -- 4. Департамент бизнес-анализа
    SELECT 
        'Департамент бизнес-анализа' AS department_name,
        4 AS order_column,
        SUM(d.current) AS total_employees_current,
        SUM(d.prev) AS total_employees_prev,
        SUM((SELECT COUNT(*) FROM results r WHERE r.q1 = d.id)) AS current_participants,
    ...
    -- 5. Проектный офис
    SELECT 
        'Проектный офис' AS department_name,
        5 AS order_column,
        SUM(d.current) AS total_employees_current,
        SUM(d.prev) AS total_employees_prev,
        SUM((SELECT COUNT(*) FROM results r WHERE r.q1 = d.id)) AS current_participants,
        SUM((SELECT COUNT(*) FROM prev_results pr WHERE pr.q1 = d.id)) AS previous_participants
    FROM departments d
    WHERE d.id IN (12, 13, 14)

    UNION ALL

    -- 6. Проектный офис - Отдел внедрения
    SELECT 
        'Проектный офис - Отдел внедрения' AS department_name,
        6 AS order_column,
        SUM(d.current) AS total_employees_current,
        SUM(d.prev) AS total_employees_prev,
        SUM((SELECT COUNT(*) FROM results r WHERE r.q1 = d.id)) AS current_participants,
    ...
    -- 7. Служба персонала
    SELECT 
        'Служба персонала' AS department_name,
        7 AS order_column,
        SUM(d.current) AS total_employees_current,
        SUM(d.prev) AS total_employees_prev,
        SUM((SELECT COUNT(*) FROM results r WHERE r.q1 = d.id)) AS current_participants,
    ...
    -- 8. Служба бизнес-процессов
    SELECT 
        'Служба бизнес-процессов' AS department_name,
        8 AS order_column,
        SUM(d.current) AS total_employees_current,
        SUM(d.prev) AS total_employees_prev,
        SUM((SELECT COUNT(*) FROM results r WHERE r.q1 = d.id)) AS current_participants,
    ...
    -- 10. Руководитель департамента/службы
    SELECT 
        'Руководитель департамента/службы' AS department_name,
        10 AS order_column,
        SUM(d.current) AS total_employees_current,
        SUM(d.prev) AS total_employees_prev,
        SUM((SELECT COUNT(*) FROM results r WHERE r.q1 = d.id)) AS current_participants,
...
$questions = [
    ...
    19 => "Если я работаю хорошо и старательно, руководитель положительно отзывается обо мне (непосредственный руководитель, например TeamLeader для программиста или бизнес-аналитика, консультанта по внедрению, Специалиста по тестированию и т.д.)",
    20 => "Руководитель ценит мои заслуги, отмечает успехи (непосредственный руководитель, например TeamLeader для программиста или бизнес-аналитика, консультанта по внедрению, Специалиста по тестированию и т.д.)",
...
function generateDashData($data, $data_prev, $data_prev_prev, $departmentIds, $departmentNameRu, $departmentNameEn, $questions, $employees, $percentages)
{
    if (count($departmentIds) > 0) {
        // Убедимся, что $departmentIds всегда массив
        if (!is_array($departmentIds)) {
            $departmentIds = [$departmentIds];
        }

        // Проверяем, есть ли данные для текущего департамента
        $hasDataForDepartment = false;

        foreach ($data as $row) {
            // Проверяем, что индекс 1 существует (столбец q1) и идентификатор департамента есть в данных
            if (isset($row[1]) && in_array($row[1], $departmentIds)) {
                $hasDataForDepartment = true;
                break;
            }
        }

        // Если данных для департамента нет, завершить выполнение функции
        if (!$hasDataForDepartment) {
            return null;
        }
    }
    global $db;
    ...
    $involvement_total_current = round($total_score * 100 / $questions_count_total / $people_amount, 2);
    ...
    if (count($departmentIds) > 0) {
        echo '<div class="accordion-item">
                <h2 class="accordion-header" id="heading' . $departmentNameEn . '">
                    <button class="accordion-button collapsed" type="button" data-bs-toggle="collapse" data-bs-target="#collapse' . $departmentNameEn . '" aria-expanded="true" aria-controls="collapse' . $departmentNameEn . '">
                        ' . $departmentNameRu . ' (Вовлеченность: ' . $involvement_total_current . '%; Участники: ' . $participationRate . '%; eNPS: ' . $eNPS . '%)
                    </button>
                </h2>
        ...
        echo '<div class="accordion-item">
                <h2 class="accordion-header" id="heading' . $departmentNameEn . '">
                    <button class="accordion-button collapsed" type="button" data-bs-toggle="collapse" data-bs-target="#collapse' . $departmentNameEn . '" aria-expanded="true" aria-controls="collapse' . $departmentNameEn . '">
                        ' . $departmentNameRu . ' (Вовлеченность: ' . $involvement_total_company . '%; Участники: ' . $participationRateCompany . '%; eNPS: ' . $eNPSCompany . '%)
                    </button>
                </h2>
                <div id="collapse' . $departmentNameEn . '" class="accordion-collapse collapse" aria-labelledby="heading' . $departmentNameEn . '" data-bs-parent="#accordionExample">
                    <div class="accordion-body">';
    ...
    if (count($departmentIds) > 0 && $departmentIds != [9]) {
        echo "<div class='square'>
            <u><h3>Вовлеченность " . $departmentNameRu . "  (" . $survey_titles['title_current'] . "/" . $survey_titles['title_prev'] . ")</h3></u>
            <h1>" . $involvement_total_current . "%/" . $involvement_total_prev . "%</h1>
        </div>";
    } else if($departmentIds ==[9]){
        echo "<div class='square'>
            <u><h3>Вовлеченность " . $departmentNameRu . "  (" . $survey_titles['title_current'] . ")</h3></u>
            <h1>" . $involvement_total_current . "%</h1>
        </div>";
    }
    echo "<div class='square'>
    ...
    if (count($departmentIds) > 0 && $departmentIds != [9]) {
        echo '<div class="graph' . $departmentNameEn . '3">
            <canvas class="differenceGraph" id="chart' . $departmentNameEn . '3" width="100" height="50"></canvas>
        </div>';
    } else if(count($departmentIds) == 0 && $departmentIds != [9]){
    ...
    echo '<div class="graph' . $departmentNameEn . '2">
        <canvas class="questionGraph" id="chart' . $departmentNameEn . '2"></canvas>
    </div>';
    echo '
    ...
    if ($departmentIds == [18]) {
            ...
            options: {
                ...
                plugins: {
                    ...
                    datalabels: { // Настройки плагина DataLabels
                        ...
                        color: "#434343", // Цвет текста
                        font: {
                            weight: "bold"
                        }
                    }
                },
                barPercentage: 0.9, // Уменьшенная ширина столбцов
        ...
        const chart' . $departmentNameEn . '1 = new Chart(ctx' . $departmentNameEn . '1, {
            type: "bar",
            data: {
                labels: ["' . $survey_titles['title_current'] . '"], // Метки для каждого критерия
                datasets: [
                    {
                        label: "ОВ",
                        data: [
    ...
    echo 'const ctx' . $departmentNameEn . '2 = document.getElementById("chart' . $departmentNameEn . '2").getContext("2d");
        ...
        const questionColors' . $departmentNameEn . ' = departmentData.map(value => value < 70 ? "red" : "#434343");
    ...
    if (count($departmentIds) > 0) {
        echo 'datasets: [{
                            ...
                            borderColor: "#999B9A",
                            borderWidth: 0,
                            barThickness: 20, // Уменьшаем ширину столбцов
                        },
                        {
    ...
    echo '      options: {
                    ...
                    scales: {
                        ...
                        y: {
                            ...
                            ticks: {
                                font: {
                                    size: 14, // Уменьшенный шрифт для меток
                                },
                                color: function(context) {
                                    // Установка цвета текста
                                    const index = context.index;
                                    return questionColors' . $departmentNameEn . '[index] || "#434343";
                                },
                                callback: function(value, index) {
                                    const label = this.getLabelForValue(value);
                                    const maxLineLength = 90; // Максимальная длина строки
                    ...
                    elements: {
                        bar: {
                            barThickness: 30, // Увеличенная ширина столбцов
                        }
                    },
                    categoryPercentage: 0.1, // Уменьшить для увеличения отступов между группами столбцов
                    barPercentage: 0.3,
                },
                plugins: [
                    ChartDataLabels,
                    {
                        ...
                        beforeDraw: (chart) => {
                            ...
                            yAxis.ticks.forEach((tick, index) => {
                                ...
                                const color = questionColors' . $departmentNameEn . '[index] || "#434343";
            ...
            options: {
                ...
                scales: {
                    y: {
                        ...
                        ticks: {
                            font: {
                                size: 14, // Устанавливаем шрифт 8px для labels 
                            },
                            color: "#434343",
                            callback: function(value, index) {
                                const label = this.getLabelForValue(value);
                    ...
                    x: {
                        ...
                        suggestedMin: Math.min(...sortedDifferences) * 1.4, // Увеличиваем максимальное значение на оси x
                    }
                },
                plugins: {
                    tooltip: {
            ...
            plugins: [
                {
                    ...
                    afterDatasetsDraw: (chart) => {
                        ...
                        chart.data.datasets[0].data.forEach((value, index) => {
                            ...
                            ctx.save();
                            ctx.font = "bold 12px Roboto"; // Шрифт 12px для процентов
                            ctx.fillStyle = "#434343"; // Чёрный цвет для текста процентов
                    ...
                    options: {
                        ...
                        plugins: {
                            ...
                            datalabels: {
                                ...
                                // Форматирование и обработка значений лейблов
                                formatter: function(value, context) {
                                    // Получаем значение столбца для текущего столбца
                                    const columnValue = context.dataset.data[context.dataIndex];
                
                                    return columnValue + "%"; // Добавляем знак процента
                                },
                                anchor: "end",  // Лейбл в верхней части столбца
                                align: "top",   // Выравнивание по верхнему краю
                                offset: function(context) {
                                    const columnValue = context.dataset.data[context.dataIndex]; // Получаем значение столбца
                
                                    // Если значение >= 95%, помещаем лейбл внутри столбца
                                    if (columnValue >= 95) {
                                        return -30;  // Отступ внутрь столбца
                                    } else {
                                        return 5;  // Отступ над столбцом
                                    }
                                }
                            }
                        }
...
<div class="accordion" id="accordionExample">
    ...
    $departmentIds = [3, 4, 5, 7];
    $departmentNameRu = "Производственный департамент";
    $departmentNameEn = "PrDep";
    generateDashData($data, $data_prev, $data_prev_prev, $departmentIds, $departmentNameRu, $departmentNameEn, $questions, $employees, $percentages);

    $departmentIds = [3];
    $departmentNameRu = "Производственный департамент - Группы разработки";
    $departmentNameEn = "PrDepGrDev";
    generateDashData($data, $data_prev, $data_prev_prev, $departmentIds, $departmentNameRu, $departmentNameEn, $questions, $employees, $percentages);

    $departmentIds = [4];
    $departmentNameRu = "Производственный департамент - Отдел тестирования";
    $departmentNameEn = "PrDepTest";
    generateDashData($data, $data_prev, $data_prev_prev, $departmentIds, $departmentNameRu, $departmentNameEn, $questions, $employees, $percentages);

    $departmentIds = [5];
    $departmentNameRu = "Производственный департамент - Служба сопровождения";
    $departmentNameEn = "PrDepEscort";
    generateDashData($data, $data_prev, $data_prev_prev, $departmentIds, $departmentNameRu, $departmentNameEn, $questions, $employees, $percentages);

    $departmentIds = [7];
    $departmentNameRu = "Производственный департамент - Другое";
    $departmentNameEn = "PrDepEtc";
    generateDashData($data, $data_prev, $data_prev_prev, $departmentIds, $departmentNameRu, $departmentNameEn, $questions, $employees, $percentages);

    $departmentIds = [8, 9, 10];
    $departmentNameRu = "Департамент развития бизнеса";
    $departmentNameEn = "BusProdDep";
    generateDashData($data, $data_prev, $data_prev_prev, $departmentIds, $departmentNameRu, $departmentNameEn, $questions, $employees, $percentages);

    $departmentIds = [8];
    $departmentNameRu = "Департамент развития бизнеса - Аккаунт менеджер";
    $departmentNameEn = "BusProdAcc";
    generateDashData($data, $data_prev, $data_prev_prev, $departmentIds, $departmentNameRu, $departmentNameEn, $questions, $employees, $percentages);

    $departmentIds = [9];
    $departmentNameRu = "Департамент развития бизнеса и продуктов - Пресейл консультант";
    $departmentNameEn = "BusProdPresale";
    generateDashData($data, $data_prev, $data_prev_prev, $departmentIds, $departmentNameRu, $departmentNameEn, $questions, $employees, $percentages);

    $departmentIds = [10];
    $departmentNameRu = "Департамент развития - Маркетинг";
    $departmentNameEn = "BusProdEtc";
    generateDashData($data, $data_prev, $data_prev_prev, $departmentIds, $departmentNameRu, $departmentNameEn, $questions, $employees, $percentages);
    ...
    // $departmentIds = [12, 13];
    // $departmentNameRu = "Проектный офис - Менеджеры";
    // $departmentNameEn = "PMOManagers";
    // generateDashData($data, $data_prev, $data_prev_prev, $departmentIds, $departmentNameRu, $departmentNameEn, $questions, $employees, $percentages);
    ...
    $departmentNameEn = "backoffice";
    generateDashData($data, $data_prev, $data_prev_prev, $departmentIds, $departmentNameRu, $departmentNameEn, $questions, $employees, $percentages);

    $departmentIds = [19];
    $departmentNameRu = "Не хочу отвечать";

...
<body>
    ...
    <div class="container">
        <div class="first">
            ...
            <table>
                ...
                <tbody>
                    ...
                    $count = 0;
                    foreach ($data as $item) {
                        if ($item[1] == 12 || $item[1] == 13 || $item[1] == 14) {
                            echo "<tr>";
                            for ($i = 0; $i <= 24; $i++) {
                                echo ("<td>" . $item[$i] . "</td>");
                            }
            ...
            foreach ($sum_arr as $key => $item) {
                $questionText = $questions[$key] ?? "Неизвестный вопрос"; // заменяем индекс на текст вопроса
                $PMO_curr_avg_arr[$questionText] = round($item / $count * 100, 2);
            }
        ...
        <div class="second">
            ...
            foreach ($sum_arr as $key => $item) {
                $questionText = $questions[$key] ?? "Неизвестный вопрос"; // заменяем индекс на текст вопроса
                $PMO_prev_avg_arr[$questionText] = round($item / $count * 100, 2);
            }
...
<script>
    ...
    const PMOchartData = <?php echo json_encode($PMOchartData); ?>;
    const PMOcurrentData = <?= json_encode(array_values($PMO_curr_avg_arr)) ?>;
    const PMOpreviousData = <?= json_encode(array_values($PMO_prev_avg_arr)) ?>;
    // Предполагаем, что currentData и previousData имеют одинаковую длину
    let PMOsortedData = labels.map((label, index) => ({
        label: label,
        PMOcurrentValue: PMOcurrentData[index],
        PMOpreviousValue: PMOpreviousData[index],
    }));

    // Сортируем по текущим значениям в порядке убывания
    PMOsortedData.sort((a, b) => b.PMOcurrentValue - a.PMOcurrentValue);

    // Создаем новые массивы для меток и данных
    const PMOsortedLabels = PMOsortedData.map(item => item.label);
    const PMOsortedCurrentData = PMOsortedData.map(item => item.PMOcurrentValue);
    const PMOsortedPreviousData = PMOsortedData.map(item => item.PMOpreviousValue);
    ...
    document.addEventListener("DOMContentLoaded", function() {
        ...
        const myChart3 = new Chart(ctx3, {
            ...
            data: {
                ...
                datasets: [{
                        label: 'Предыдущий период',
                        data: Object.values(PMOchartData.previous),
                        backgroundColor: '#2E5B9B', // Цвет для предыдущего периода
                        borderColor: '#2E5B9B',
                        borderWidth: 1
                    },
                    {
                        label: 'Текущий период',
                        data: Object.values(PMOchartData.current),
                        backgroundColor: '#999B9A', // Основной цвет столбцов для текущего периода
                        borderColor: '#999B9A',
                        borderWidth: 1
                    }
